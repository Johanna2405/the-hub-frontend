            ...
            <div className="flex gap-2 overflow-x-auto mb-6">
                {days.map(({ day, label }) => (
                    <div
                        ...
                        className={`flex flex-col items-center border rounded-xl px-4 py-2 cursor-pointer transition-all duration-200 w-16 min-w-[64px] ${selectedDay === day ? "bg-base" : "bg-white"}`}
                    >
                ...
                return (
                    <div key={day} className={`rounded-3xl p-6 mb-4 ${bg}`}>
                        <div className="flex justify-between items-center mb-2">
                            ...
                            <button onClick={() => setExpanded(expanded === day ? null : day)}>
                                <i className={`fi-rr-angle-small-${expanded === day ? "up" : "down"} text-text`}></i>
                            </button>
                        </div>
                        ...
                        {showEvents && (
                            <div className="flex flex-col gap-4">
                                {events.map((event, i) => (
                                    ...
                                    >
                                        <div>
                                            {event.time && <p className={`text-m mb-1 ${day === "25" ? "text-white" : "text-text"}`}>{event.time}</p>}
                                            <h4 className="text-text font-light">{event.title}</h4>
                                        </div>
                                        <IconBtn color="base" icon="fi-rr-plus-small" />
                                    </div>
                                ))}
                                {isToday && (
                                    <a className="text-text font-bold mt-2 cursor-pointer">Go to Day</a>
                                )}
                            </div>
                        )}
                    </div>
